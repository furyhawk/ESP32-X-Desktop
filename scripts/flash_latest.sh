...
Options:
  ...
  -d, --dir DIR        Directory to search for latest flash_*.bin.gz (default: current directory).
    -p, --port PORT      Serial port (default: auto-detect /dev/ttyACM*, /dev/ttyUSB*, /dev/tty.usbmodem*, or /dev/tty.usbserial*).
...
detect_flash_size_bytes() {
    ...
    local tmp
    tmp="$(mktemp)"
    if "$ESPTOOL" --chip "$CHIP" --port "$PORT" --baud "$BAUD" --before default_reset --after no_reset flash_id 2>&1 | tee "$tmp" >/dev/null; then
        :
    else
        rm -f "$tmp"
        return 1
...
pick_latest_image() {
    ...
    for pat in "flash_*.bin.gz" "*.bin.gz" "flash_*.bin" "*.bin"; do
        ...
        for f in "$dir"/$pat; do
            [[ -f "$f" ]] || continue
            mtime="$(file_mtime_epoch "$f" 2>/dev/null || echo 0)"
...
if [[ "$BIN_SRC" == *.gz ]]; then
    ...
    TEMP_BIN="$(mktemp -t flash_latest_bin_XXXXXX)"
...
# Determine sizes
BIN_SIZE="$(file_size_bytes "$BIN_TO_FLASH" 2>/dev/null || echo 0)"
...
if [[ "$ERASE" == "1" ]]; then
    ...
    set -x
    "$ESPTOOL" --chip "$CHIP" --port "$PORT" --baud "$BAUD" --before default_reset --after no_reset erase_flash
    { set +x; } 2>/dev/null
fi
