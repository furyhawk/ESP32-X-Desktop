...
static bool wifi_scan_result_is_hidden(const wifi_ap_record_t *ap)
{
    if (!ap) {
        return true;
    }

    return (strnlen((const char *)ap->ssid, sizeof(ap->ssid)) == 0);
}
...
uint16_t network_prov_mgr_wifi_scan_result_count(void)
{
    ...
    uint16_t idx = 0;
    while (idx < MAX_SCAN_RESULTS) {
        const wifi_ap_record_t *ap = prov_ctx->ap_list_sorted[idx];
        if (!ap) {
            break;
        }
        if (!wifi_scan_result_is_hidden(ap)) {
            rval++;
        }
        idx++;
    }
...
const wifi_ap_record_t *network_prov_mgr_wifi_scan_result(uint16_t index)
{
    ...
    uint16_t visible_idx = 0;
    for (uint16_t idx = 0; idx < MAX_SCAN_RESULTS; idx++) {
        const wifi_ap_record_t *candidate = prov_ctx->ap_list_sorted[idx];
        if (!candidate) {
            break;
        }
        if (wifi_scan_result_is_hidden(candidate)) {
            continue;
        }
